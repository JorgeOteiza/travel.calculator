...
const ResultDetails = () => {
  ...
  return <div className="result-page">
    ...
    <section className="metric-grid"><article><span>Distancia</span><strong>{formatDistance(distance)} {distanceUnit}</strong><small>Ruta calculada</small></article><article><span>Combustible</span><strong>{formatLiters(result.fuelUsed)} L</strong><small>{formatConsumption(result.adjustedFC)} L/100 km</small></article><article className="metric-highlight"><span>Costo estimado</span><strong>{cost}</strong><small>Precio ingresado</small></article><article><span>Clima</span><strong>{result.weather}</strong><small>Pendiente media {formatPercentage(result.roadGrade)}%</small></article></section>
    ...
    <section className="result-detail-grid"><article><h2>Vehículo</h2><dl><div><dt>Marca y modelo</dt><dd>{result.vehicle?.make || "-"} {result.vehicle?.model || ""}</dd></div><div><dt>Año</dt><dd>{result.vehicle?.year || "-"}</dd></div><div><dt>Motor</dt><dd>{result.vehicle?.engine_cc ? `${formatNumber(result.vehicle.engine_cc, { maximumFractionDigits: 0 })} cc` : "-"}</dd></div><div><dt>Consumo base</dt><dd>{formatConsumption(result.baseFC)} L/100 km</dd></div></dl></article><article><h2>Cómo se calculó</h2><ol><li>Consumo base del vehículo.</li><li>Tipo de vía: {result.roadProfile || "mixed"}.</li><li>Pasajeros y carga adicional.</li><li>{result.segmentsAnalyzed || 0} segmentos de ruta.</li><li>Elevación {result.elevationSource === "flat_fallback" ? "con fallback plano" : "Open-Meteo / Copernicus DEM"}.</li><li>Condiciones climáticas.</li>{result.operatingConditions?.departure_hour != null && <li>Tráfico {result.operatingConditions.traffic_level} a las {String(result.operatingConditions.departure_hour).padStart(2, "0")}:00{result.operatingConditions.is_short_trip ? ", incluyendo ajuste por trayecto corto y arranque en frío" : ""}.</li>}</ol>{result.operatingConditions?.departure_hour != null && <p className="data-attribution">Tráfico estimado por horario; no corresponde a información en tiempo real.</p>}{result.elevationSource === "open_meteo" && <p className="data-attribution">Elevación: Open-Meteo, Copernicus DEM GLO-90.</p>}</article></section>
